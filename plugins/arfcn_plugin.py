...
class ArfcnPlugin(PluginBase):
    ...
    @arg('-b', action="store", dest="band", choices=(arfcn.get_bands()), help="Frequency band of the ARFCN.")
    ...
    def channelinfo(self, args):
        ...
        if args.arfcn is not None:
            if args.band is not None:
                ...
                for b in arfcn.get_bands():
                    self.__print_arfcn_info(args.arfcn, b)
        else:
            if args.band is not None:
                # print the starting channels and end channels of the band
                arfcn_range = arfcn.get_arfcn_ranges(args.band)[0]
                arfcn_start = arfcn_range[0]
                f_down_start = arfcn.arfcn2downlink(arfcn_start, args.band) / 1e6
                f_up_start = arfcn.arfcn2uplink(arfcn_start, args.band) / 1e6

                arfcn_end = arfcn_range[1]
                f_down_end = arfcn.arfcn2downlink(arfcn_end, args.band) / 1e6
                f_up_end = arfcn.arfcn2uplink(arfcn_end, args.band) / 1e6
                ...
                for band in arfcn.get_bands():
                    arfcn_range = arfcn.get_arfcn_ranges(band)[0]
                    # print the starting channels and end channels of the band
                    arfcn_start = arfcn_range[0]
                    f_down_start = arfcn.arfcn2downlink(arfcn_start, band) / 1e6
                    f_up_start = arfcn.arfcn2uplink(arfcn_start, band) / 1e6

                    arfcn_end = arfcn_range[1]
                    f_down_end = arfcn.arfcn2downlink(arfcn_end, band) / 1e6
                    f_up_end = arfcn.arfcn2uplink(arfcn_end, band) / 1e6
    ...
    @arg('-b', action="store", dest="band", choices=(arfcn.get_bands()))
    ...
    def frequencyinfo(self, args):
        ...
        if args.band is not None:
            if not arfcn.is_valid_downlink(args.freq, args.band) and not arfcn.is_valid_uplink(args.freq,
                                                                                                           args.band):
                self.printmsg("Freq %4.1f MHz is NOT valid in band %s" % (args.freq, args.band))

            if arfcn.is_valid_downlink(args.freq, args.band):
                channel = arfcn.downlink2arfcn(args.freq, args.band)
                self.printmsg(
                    "Freq %4.1f MHz (Downlink) is ARFCN %i in band %s" % (args.freq / 1e6, channel, args.band))
            if arfcn.is_valid_uplink(args.freq, args.band):
                channel = arfcn.uplink2arfcn(args.freq, args.band)
                self.printmsg("Freq %4.1f MHz (Uplink) is ARFCN %i in band %s" % (args.freq / 1e6, channel, args.band))
        else:
            for b in arfcn.get_bands():
                if not arfcn.is_valid_downlink(args.freq, b) and not arfcn.is_valid_uplink(args.freq, b):
                    self.printmsg("Freq %4.1f MHz is NOT valid in band %s" % (args.freq / 1e6, b))

                if arfcn.is_valid_downlink(args.freq, b):
                    channel = arfcn.downlink2arfcn(args.freq, b)
                    self.printmsg("Freq %4.1f MHz (Downlink) is ARFCN %i in band %s" % (args.freq / 1e6, channel, b))
                if arfcn.is_valid_uplink(args.freq, b):
                    channel = arfcn.uplink2arfcn(args.freq, b)
    ...
    def __print_arfcn_info(self, arfcn, band):
        if arfcn.is_valid_arfcn(arfcn, band):
            f_down = arfcn.arfcn2downlink(arfcn, band) / 1e6
            f_up = arfcn.arfcn2uplink(arfcn, band) / 1e6
